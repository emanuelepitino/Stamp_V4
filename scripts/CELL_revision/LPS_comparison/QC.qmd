...
mtx <- readRDS(glue("{proj_dir}/data/lps_pbmcs/raw/CZI.PBMC.RNA.matrix.Rds"))
adt <- readRDS(glue("{proj_dir}/data/lps_pbmcs/raw/CZI.PBMC.ADT.matrix.Rds"))

bcd <- data.table::fread(glue("{proj_dir}/data/lps_pbmcs/raw/CZI.PBMC.cell.annotations.csv"), header = T)

sce <- SingleCellExperiment(assays = list(counts = mtx), colData = DataFrame(bcd))
sce
sce$Sample <- "LPS"
```
...
```{r}
dir.create(glue("{proj_dir}/data/LPS_pbmcs/QC"), recursive = T, showWarnings = F)
qsave(sce, file = glue("{proj_dir}/data/LPS_pbmcs/QC/QC1.qs"), nthreads = 8)
```
