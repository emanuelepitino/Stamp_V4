...
## Load data
```{r}
myeloid <- qread(glue("{proj_dir}/data/stamp_5/processed/Lvl2/Myeloid/Myeloid/lvl2_sce.qs"), nthreads = 8)
B <- qread(glue("{proj_dir}/data/stamp_5/processed/Lvl2/B/B/lvl2_sce.qs"), nthreads = 8)
T <- qread(glue("{proj_dir}/data/stamp_5/processed/Lvl2/T/lvl2_sce.qs"), nthreads = 8)
```

```{r}
sce <- cbind(myeloid,B,T)
```

```{r}
sce$Cell_Lineage <- sce$lvl1
sce$Cell_Type <- sce$lvl2
sce$Cell_Lineage <- factor(sce$Cell_Lineage, levels = c("T","B","Myeloid"))
levels(sce$Cell_Type)[levels(sce$Cell_Type) == "Th2"] <- "Th"
```
...
qc_metrics_anno <- wrap_plots(
    plot_density(df, "sum", "Cell_Type", pal, "Counts",200),
    plot_density(df, "detected", "Cell_Type", pal, "Features",150),
    plot_density(df, "cell_area", "Cell_Type", pal, "Cell Area",500),
    plot_density(df, "nucleus_area", "Cell_Type", pal, "Nucleus Area",500),
    ncol = 2, nrow = 2) +
plot_annotation(tag_levels = "A")
```
...
```{r}
feats_lin <- c("CD3E","CD3D","CD247","NGK7","NCAM1",
           "CD19","CD79A","CD79B",
           "CD14","FCGR3A","CD68","CD86")
```
...
```{r}
sce$Cell_Type <- factor(sce$Cell_Type, levels = c("Naive CD4","Memory CD4","Th","Tregs","Naive CD8",
                                         "Memory CD8","Cytotoxic CD8",
                                        "CD56dim CD16+ NK",
                                         "CD56+ CD16- NK",
                                        "Naive B","Class Switched B", "Naive IgK+ B", "Class Switched IgK+ B", 
                                        "Plasmacells",
                                        "CD14+ Mono","Int Mono", "CD16+ Mono", "CD1C+ DCs",
                                        "LowQ"))
```
