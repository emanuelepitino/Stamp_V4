...
## Clusters analysis
```{r}
gg_clust1 <- create_plots2(sce, "label", intersect(feats_xenium, rownames(sce)), pal)
...
sce$lvl1[sce$label == "5"] <- "B"
sce$lvl1[sce$label == "6"] <- "B"
sce$lvl1[sce$label == "7"] <- "T" # i know it's NK, but we want to group with T for the moment
...
```{r}
# A
gg_anno1 <- create_plots2(sce, "lvl1", intersect(feats_xenium, rownames(sce)), pal_lvl1)
