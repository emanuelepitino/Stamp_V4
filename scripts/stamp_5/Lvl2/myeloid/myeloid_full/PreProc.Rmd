...
```{r}
t_from_b <- qread(glue("{proj_dir}/data/stamp_5/processed/Lvl2/Mislabeled/Myeloid_from_B.qs"))
```

```{r}
sce <- cbind(sce,t_from_b)
```
...
sub_to_remove <- "T"
sub <- "Myeloid"
sce <- sce[,sce$lvl2!= sub_to_remove]
sce
```
...
qsave(sce, glue("{res_dir}/proc_sce2.qs"))
qsave(dec.var, glue("{res_dir}/dec_var2.qs"))
```
