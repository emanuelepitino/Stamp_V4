...
feats <- c("CD19","CD79A","CD79B","IGHM","IGKC",
           ...
           "CD3E","TCF7","IL7R","NKG7","KLRK1","GNLY","S100A9","ITGAX","CLEC12A")
```
...
# Annotation
```{r}
sce$lvl2[sce$label == 1] <- "Class Switched B"
sce$lvl2[sce$label == 5] <- "Plasmacells"
sce$lvl2[sce$label == 10] <- "NK"
sce$lvl2[sce$label == 2] <- "Naive IgK+ B"
sce$lvl2[sce$label == 3] <- "Naive B"
sce$lvl2[sce$label == 4] <- "Naive B"
sce$lvl2[sce$label == 6] <- "Naive B"
sce$lvl2[sce$label == 7] <- "LowQ"
sce$lvl2[sce$label == 8] <- "Myeloid"
sce$lvl2[sce$label == 9] <- "T"
```
...
```{r}
sce$lvl2 <- factor(sce$lvl2, levels = c("Naive B", "Naive IgK+ B", "Class Switched B", 
                                        "Plasmacells","T","NK","Myeloid","LowQ"))
```
...
```{r}
# Plot without the contamination of T NK Myeloid
sce_clean <- sce[,sce$lvl2 != "T" & sce$lvl2 != "NK" & sce$lvl2 != "Myeloid"]
...
# Save cell id of the T NK Myeloid cells
```{r}
t <- sce[,sce$lvl2 == "T"]
nk <- sce[,sce$lvl2 == "NK"]
myeloid <- sce[,sce$lvl2 == "Myeloid"]

qsave(t, file = glue("{proj_dir}/data/stamp_5/processed/Lvl2/Mislabeled/T_from_{sub}.qs"))
qsave(nk, file = glue("{proj_dir}/data/stamp_5/processed/Lvl2/Mislabeled/NK_from_{sub}.qs"))
qsave(myeloid, file = glue("{proj_dir}/data/stamp_5/processed/Lvl2/Mislabeled/Myeloid_from_{sub}.qs"))
```
