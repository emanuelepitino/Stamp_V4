...
feats <- c("CD3E","CD3D","CD247","CD4","CCR7","SELL","TCF7","IL7R","GATA3","FOXP3","CTLA4",
           "IL2RA","CD8A","CD8B","PRF1","GZMB","GZMA","FCGR3A","NCAM1","NGK7","GZMK")
```
...
sce$lvl2[sce$label == 10] <- "Memory CD8"
sce$lvl2[sce$label == 11] <- "Cytotoxic CD8"
sce$lvl2[sce$label == 12] <- "CD56dim CD16dim NK"
sce$lvl2[sce$label == 13] <- "Central Memory CD4"
sce$lvl2[sce$label == 14] <- "Naive CD4"
sce$lvl2[sce$label == 15] <- "Central Memory CD4"
sce$lvl2[sce$label == 2] <- "Th2"
sce$lvl2[sce$label == 3] <- "Central Memory CD4"
sce$lvl2[sce$label == 4] <- "Tregs"
sce$lvl2[sce$label == 5] <- "Central Memory CD4"
sce$lvl2[sce$label == 6] <- "LowQ"
sce$lvl2[sce$label == 7] <- "CD56+ CD16+ NK"
sce$lvl2[sce$label == 8] <- "CD56+ CD16dim NK"
sce$lvl2[sce$label == 9] <- "Naive CD8"
```

```{r}
sce$lvl2 <- factor(sce$lvl2, levels = c("Naive CD4","Central Memory CD4","Th2","Tregs","Naive CD8",
                                         "Memory CD8","Cytotoxic CD8",
                                        "CD56dim CD16dim NK",
                                         "CD56+ CD16+ NK","CD56+ CD16dim NK","LowQ"))
```
