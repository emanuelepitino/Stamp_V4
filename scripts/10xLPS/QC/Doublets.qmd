---
title: "Doublets scoring - LPS PBMCs"
...
sce <- qread(glue("{proj_dir}/data/10xLPS/QC/QC1_lps.qs"), nthreads = 8)
```
...
hvg <- getTopHVGs(dec.var,fdr.threshold = 1) # find variable features by fdr 
length(hvg)
hvg <- hvg[1:5000]
length(hvg)
```
...
# Save
```{r}
qsave(sce,file = glue("{proj_dir}/data/10xLPS/QC/doublets_lps_sce.qs"), nthreads = 8)
```
