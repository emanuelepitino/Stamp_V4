---
title: "Exploratory - clustering P2 (Clean)- Stamp 3 - PBMCs"
...
unsup <- qread(glue("{dir}/CleanUnsup.qs"))
```
...
```{r}
sce$lvl1[sce$label == "a"] <- "DCs"
sce$lvl1[sce$label == "b"] <- "Plasmablasts"
sce$lvl1[sce$label == "c"] <- "T"
sce$lvl1[sce$label == "d"] <- "Myeloid"
sce$lvl1[sce$label == "e"] <- "Myeloid"
sce$lvl1[sce$label == "e"] <- "Myeloid"
sce$lvl1[sce$label == "f"] <- "T"
sce$lvl1[sce$label == "g"] <- "NK"
sce$lvl1[sce$label == "g"] <- "NK"
sce$lvl1[sce$label == "h"] <- "Myeloid"
sce$lvl1[sce$label == "i"] <- "B"
...
gg_um <- ggplot(df, aes(x = V1, y = V2, color = lvl1)) +
...
```{r}
dir <- glue("{proj_dir}/data/{stamp}/{sample}/Ist")
qsave(sce, file = glue("{dir}/lvl1_sce.qs"), nthreads = 8)
```
