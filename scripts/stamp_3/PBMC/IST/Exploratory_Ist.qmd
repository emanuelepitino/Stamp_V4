...
ggplot(df, aes(x = PC1, y = PC2, color = log1p(CenterX_global_px))) +
...
gg_box <- \(var){
p <- ggplot(df, aes(x = label, y = !!sym(var), color = label)) +
  ...
  if(var == "sum" | var == "detected" | var == "Mean.DAPI") {p <- p + scale_y_log10()}
return(p)
}
