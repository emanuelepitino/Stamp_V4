---
title: "Quality Control - Stamp 3 - PBMCs"
...
data_dir <- glue("{proj_dir}/data/stamp_3/raw/raw_proc")
```
...
cd_rotated <- cd %>%
  mutate(
    rotated_coords = as.data.frame(t(rotation_matrix %*% t(as.matrix(select(cd, CenterX_global_px, CenterY_global_px))))),
    CenterX_rotated = rotated_coords$V1,
    CenterY_rotated = rotated_coords$V2
  ) %>%
  select(-rotated_coords)

# Plot the rotated points
ggplot(cd_rotated, aes(CenterX_rotated, CenterY_rotated, color = nCount_RNA)) + 
  geom_point(shape = 16, size = 0.001) + 
  scale_color_viridis_c() +
  coord_equal()

ggplot(cd_rotated, aes(CenterX_rotated, CenterY_rotated, color = nFeature_RNA)) + 
  geom_point(shape = 16, size = 0.001) + 
  scale_color_viridis_c() +
  coord_equal()

ggplot(cd_rotated, aes(CenterX_rotated, CenterY_rotated, color = Area.um2)) + 
  geom_point(shape = 16, size = 0.001) + 
  scale_color_viridis_c() +
  coord_equal()
```
...
```{r}
dir <- glue("{plt_dir}/stamp_3/{sample}")
...
```{r}
dir <- glue("{proj_dir}/data/stamp_3/{sample}")
