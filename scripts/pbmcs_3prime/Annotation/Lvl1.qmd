...
sce$lvl1[sce$label == "1"] <- "Doublets"
sce$lvl1[sce$label == "2"] <- "CD8"
sce$lvl1[sce$label == "3"] <- "CD4"
sce$lvl1[sce$label == "4"] <- "Myeloid"
sce$lvl1[sce$label == "5"] <- "NK"
sce$lvl1[sce$label == "6"] <- "Doublets"
...
wrap_plots(
  ...
  plotDots(sce, features = c("CD3E","CD8A","CD4","CD79A","CD79B","NKG7","NCAM1","CD14","FCGR3A","CD1C"),group = "lvl1", scale = T, center = T) +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        aspect.ratio = 1/4),
  ncol = 2)
