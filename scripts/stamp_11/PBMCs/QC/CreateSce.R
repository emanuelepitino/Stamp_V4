...
barcodes <- fread(glue("{data_dir}/barcodes.tsv.gz"), sep = "\t", nThread = 8, header = F)
features <- fread(glue("{data_dir}/features.tsv.gz"), sep = "\t", nThread = 8, header = F)
counts <- Matrix::readMM(glue("{data_dir}/matrix.mtx.gz"))
