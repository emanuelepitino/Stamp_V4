...
# Filter FOVs with only specific sub-stamps
```{r}
...
plot_d_border <- function(df, title, thr) {
  ...
  p <- ggplot(df, aes(x = d_border_px, y = nCount_RNA)) +
    geom_line(data = bin_data, aes(x = bin_centers, y = bin_means), color = "black") +
...
```{r}
gg_border_effect <- wrap_plots(a,b,c,d, ncol = 2, nrow = 2) + plot_annotation(title = glue("Border effect on {stamp} - iPSCs"))
```
...
wrap_plots(gg_border_effect, gg_border_eff_quantification, nrow = 2) +
  plot_annotation(title  = glue("Border effect - {stamp} - iPSCs"),
