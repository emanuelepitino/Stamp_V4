---
title: "Quality control - Stamp 13a - PBMCs perturbations"
...
```{r}
sce$sum_area <- sce$sum/sce$Area.um2
```
...
gg_spat_sum_area <- plot_metrics(df, "sum_area")

gg_spat_metrics <- wrap_plots(gg_spat_counts,gg_spat_feat,
                              gg_spat_area,gg_spat_sum_area,
                              ncol = 2,nrow = 2) + 
...
df$color <- "no"
df$color[df$ncells < 650] <- "yes"

ggplot(df, aes(x = reorder(fov, -ncells), y = ncells, fill = color)) +
  geom_col() +
  scale_fill_manual(values = c("no" = "grey", yes = "red4")) +
  theme_bw() +
...
```{r}
cd <- as.data.frame(colData(sce))
cd$filter_fov[cd$fov ]
```

## Remove affected FOVs in LPS_4h_r2
```{r}
sce <- sce[,sce$fov %!in% c(36,41,46)]
```
