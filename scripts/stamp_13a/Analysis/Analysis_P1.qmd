...
sub$id <- paste0(sub$replicate,"_",sub$lvl1,"_",sub$timepoint,"_",sub$experiment)
agg <- aggregateAcrossCells(sub, ids = sub$id, use.assay.type = "logcounts", statistics = "mean")

mds <- runMDS(agg)

mds_plt <- list()
for(sub in unique(mds$lvl1)){
  
mds_plt[[sub]]<- plotMDS(mds[,mds$lvl1 == sub], 
...
myelo <- sce[,sce$lvl0 == "myeloid" & 
               ...
               sce$timepoint == "4h"]
mrk <- scoreMarkers(sub, groups = sub$experiment, BPPARAM = bp)
