...
traj_sce <- runPCA(traj_sce, ncomponents=12)
...
```{r}
traj_milo <- buildGraph(traj_milo, k = 80, d = 10)
```
...
```{r}
traj_milo <- makeNhoods(traj_milo, prop = 0.1, k = 80, d=10, refined = TRUE)
```
...
```{r}
traj_milo <- calcNhoodDistance(traj_milo, d=10)
```
