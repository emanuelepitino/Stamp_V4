...
sce <- qread(glue("{proj_dir}/data/PBMCs_5prime/QC/doublets_sce.qs"), nthreads = 8)
```
...
# Save the workbook
dir.create(glue("{proj_dir}/Markers/PBMCs_5prime"), showWarnings = F, recursive = T)
saveWorkbook(wb, glue("{proj_dir}/Markers/PBMCs_5prime/lvl1.xlsx"), overwrite = TRUE)
```
...
wrap_plots(
  ...
  plotDots(sce, features = c("CD3E","CD8A","CD8B","CD4","CD79A","CD79B","NKG7","NCAM1","CD14","FCGR3A"),group = "label", scale = T, center = T) +
  coord_flip() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        aspect.ratio = 1/4),
  ncol = 2)
...
```{r}
dir.create(glue("{proj_dir}/data/PBMCs_5prime/Lvl1"))
qsave(sce, file = glue("{proj_dir}/data/PBMCs_5prime/Lvl1/lvl1_sce.qs"), nthreads = 8)
```
