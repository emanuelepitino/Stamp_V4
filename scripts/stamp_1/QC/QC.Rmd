...
gg_qc_metrics <- gg_qc_metrics +
                    plot_annotation(title = "Quality control - Stamp1",
...
```{r}
gg_border_effect <- wrap_plots(a,b,c,d, ncol = 2, nrow = 2) + plot_annotation(title = glue("Border effect on Stamp 1"))
```
...
```{r}
pdf(glue("{plt_dir}/stamp_1/QC1.pdf"), width = 14, height = 4 )
gg_qc_metrics
dev.off()
```
...
```{r}
pdf(glue("{plt_dir}/stamp_1/QC2.pdf"), width = 10, height = 8 )
wrap_plots(gg_border_effect, gg_border_eff_quantification, nrow = 2) +
  plot_annotation(title  = "Border effect - Stamp 1",
    subtitle = paste("Median Cells removed/FOV:", median_removed_pct,"%"),
    tag_levels = "A") +
...
```{r}
qsave(filt, glue("{proj_dir}/data/stamp_1/processed/qc_sce.qs"))
```
