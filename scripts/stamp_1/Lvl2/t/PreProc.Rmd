...
```{r}
dec.var_x <- qread(glue("{proj_dir}/data/stamp_5/processed/Lvl2/T/dec_var.qs"))
hvg <- getTopHVGs(dec.var_x,fdr.threshold =  0.9)
length(hvg)
```
...
```{r}
hvg <- getTopHVGs(dec.var,prop =  0.9)
length(hvg)
```
...
```{r}
set.seed(101001)
sce <- fixedPCA(sce, subset.row = intersect(hvg, rownames(sce)),BSPARAM=IrlbaParam())
```


```{r}
wh(6,5)
num_pcs_to_retain <- 50
