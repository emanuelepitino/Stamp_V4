---
title: "Quality control - Stamp 16 - high multiplex"
...
sce <- qread(glue("{data_dir}/layout_sce.qs"), nthreads = 8)
```
...
raw <- sce # for high_multi qcmetrics dataframe 
```
...
# QC metrics dataframe for high_multi analysis
```{r}
# Extract colData and compute raw counts per sample
cd_raw <- as.data.frame(colData(raw))
